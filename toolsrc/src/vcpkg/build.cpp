...
namespace vcpkg::Build::Command
{
                             ...
                             const VcpkgPaths& paths)
    {
        ...
        Dependencies::InstallPlanAction ipa(spec, scfl, features_as_set, Dependencies::RequestType::USER_REQUESTED, {});
...
namespace vcpkg::Build
{
    ...
    {
        const std::vector<std::string> dep_strings = get_dependency_names(scf, feature_list, triplet);
                                                             ...
                                                             const Toolset& toolset)
    {
        ...
        for (auto& feature : build_action.scfl.source_control_file->feature_paragraphs)
        {
        ...
        std::vector<System::CMakeVariable> variables{
            ...
            {"FEATURES", Strings::join(";", feature_list)},
                                                ...
                                                const Dependencies::InstallPlanAction& config)
    {
        ...
        const auto& scfl = build_action.scfl;
        ...
        if (!Strings::case_insensitive_ascii_starts_with(scfl.source_location.u8string(), paths.ports.u8string()))
        {
            System::printf("-- Installing port from location: %s\n", scfl.source_location.u8string());
        }
        ...
        System::Environment env_storage;

        const auto& env = [&]() -> const System::Environment& {
            if (env_passthrough.size() == 0)
            {
                ...
                return env_cache.get_lazy(env_cmd,
                                          [&] { return System::cmd_execute_modify_env(env_cmd, env_with_powershell); });
            }
            else
            {
                env_storage = System::cmd_execute_modify_env(
                    env_cmd,
                    System::get_environment(env_passthrough, powershell_exe_path.parent_path().u8string() + ";"));
                return env_storage;
            }
        }();
#else
        ...
        const size_t error_count =
            PostBuildLint::perform_all_checks(config.spec, paths, pre_build_info, build_info, scfl.source_location);
                                                                     ...
                                                                     const Dependencies::InstallPlanAction& config)
    {
        ...
        {
            auto& fs = paths.get_filesystem();
            const fs::path buildtrees_dir = paths.buildtrees / config.spec.name();
            auto buildtree_files = fs.get_files_non_recursive(buildtrees_dir);
            for (auto&& file : buildtree_files)
            {
                // Only delete directories, which will keep any log files
                                            ...
                                            Span<const AbiEntry> dependency_abis)
    {
        ...
        const std::string& name = config.spec.name();
    ...
    [[nodiscard]] static int decompress_archive(const VcpkgPaths& paths,
                                                ...
                                                const fs::path& archive_path)
    {
        ...
        int result = System::cmd_execute(Strings::format(R"("%s" x "%s" -o"%s" -y >nul)",
                                                         seven_zip_exe.u8string(),
                                                         archive_path.u8string(),
                                                         pkg_path.u8string()),
                                         System::get_clean_environment());
#else
        int result = System::cmd_execute(
    ...
    static void archive_failure_logs(const VcpkgPaths& paths,
                                     const fs::path& archive_tombstone_path,
                                     const fs::path& buildtree_dir,
                                     const PackageSpec& spec)
    {
        auto& fs = paths.get_filesystem();
    ...
    struct NuGetSpec
    {
        ...
        std::string nupkg_name() const { return Strings::concat(id, '.', version, ".nupkg"); }
    };

    static void archive_and_upload_nuget(const VcpkgPaths& paths,
                                         const NuGetSpec& nugetspec,
                                         const fs::path& package_dir,
                                         const Dependencies::InstallPlanAction& config,
                                         const std::vector<std::string>& nuget_dependency_strings,
...
<package>
    ...
    <files>
        ...
        auto& fs = paths.get_filesystem();
        const auto& scf = *config.build_action.value_or_exit(VCPKG_LINE_INFO).scfl.source_control_file;

        std::string nuspec_file_content = Strings::replace_all(CONTENT_TEMPLATE, "@NUGET_ID@", nugetspec.id);
        nuspec_file_content = Strings::replace_all(std::move(nuspec_file_content), "@VERSION@", nugetspec.version);
        nuspec_file_content =
            Strings::replace_all(std::move(nuspec_file_content), "@PACKAGE_DIR@", package_dir.u8string());
        nuspec_file_content =
            Strings::replace_all(std::move(nuspec_file_content),
                                 "@SUMMARY@",
        ...
        auto escaped_description = Strings::replace_all(std::string(scf.core_paragraph->description), "<", "&lt;");
        escaped_description = Strings::replace_all(std::move(escaped_description), ">", "&gt;");
        escaped_description = Strings::replace_all(std::move(escaped_description), "&", "&amp;");
        escaped_description = Strings::replace_all(std::move(escaped_description), "\"", "&quot;");
        escaped_description = Strings::replace_all(std::move(escaped_description), "\'", "&apos;");
        Strings::append(description, escaped_description, "\n\n");
        ...
        nuspec_file_content = Strings::replace_all(std::move(nuspec_file_content), "@DESCRIPTION@", description);
        ...
        const auto nuspec_path = buildtree_dir / (nugetspec.id + ".nuspec");
        ...
        else
        {
            ...
            const auto nuget_name = nugetspec.nupkg_name();
            const auto nuget_archive_path = nuget_archives / nuget_name;
            std::error_code ec;

            fs.create_directories(nuget_archives, ec);
            fs.rename(buildtree_dir / nuget_name, nuget_archive_path, VCPKG_LINE_INFO);
    ...
    static void archive_results(const VcpkgPaths& paths,
                                const fs::path& package_dir,
                                const NuGetSpec& nugetspec,
                                const fs::path& archive_path,
                                const Dependencies::InstallPlanAction& config,
                                const std::vector<std::string>& nuget_dependency_strings)
    {
        ...
        {
            archive_and_upload_nuget(paths, nugetspec, package_dir, config, nuget_dependency_strings, *feed);
        }
        else
        {
            const auto buildtree_dir = paths.buildtrees / config.spec.name();
                                      ...
                                      const StatusParagraphs& status_db)
    {
        ...
        const auto& scf = *build_action.scfl.source_control_file;
        const std::string& name = config.spec.name();
        auto triplet = config.spec.triplet();

        std::vector<FeatureSpec> required_fspecs =
            compute_required_feature_specs(scf, triplet, config.feature_list, status_db);
        ...
        if (auto abi_tag_and_file = config.abi.get())
        {
            ...
            NuGetSpec nugetspec{nuget_id, config.nuget_package_version()};
            const auto nuget_archive_path = nuget_archives / nugetspec.nupkg_name();
            ...
            if (!restored_from_cache)
            {
                if (fs.exists(archive_path))
                {
                    ...
                    auto result = decompress_archive(paths, config.spec, archive_path);
                    if (result != 0)
                    {
