    ...
    const std::string& InstallPlanAction::version() const
    {
        switch (plan_type)
        {
            ...
            case InstallPlanType::BUILD_AND_INSTALL:
                return build_action.value_or_exit(VCPKG_LINE_INFO).scfl.source_control_file->core_paragraph->version;
            default: Checks::unreachable(VCPKG_LINE_INFO);
        }
    }
                    ...
                    const fs::path& default_ports_dir)
    {
        ...
        static auto actions_to_output_string = [&](const std::vector<const InstallPlanAction*>& v) {
            return Strings::join("\n", v, [&](const InstallPlanAction* p) {
                Build::BuildPackageOptions empty_options{};
                const auto& build_opts = p->build_action ? p->build_action.get()->build_options : empty_options;
                if (auto* pscfl = p->source_control_file_location.get())
                {
                    return to_output_string(
                        p->request_type, p->displayname(), build_opts, pscfl->source_location, default_ports_dir);
                }
                else
                {
                    return to_output_string(p->request_type, p->displayname(), build_opts);
                }
            });
        };
